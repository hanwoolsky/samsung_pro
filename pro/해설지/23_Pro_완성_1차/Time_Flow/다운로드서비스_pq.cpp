...
int curTime;
priority_queue<pii, vector<pii>, greater<>> pq; // top : {end, id} 작은 값
struct Node {
...
void updateSpeed(int id, int speed) {
    if (node[id].size) {                // 1)PC인 경우
        node[id].update(speed);         //    새롭게 size, speed, start, end 설정
        pq.push({ node[id].end, id });  //    끝나는 시각 변경되므로 pq 업데이트
        return;
    }
...
void updateTime(int destTime) {
    while (pq.size()) {
        ...
        if (curTime != node[id].end || node[id].parent == -1) continue; // 유효하지 않은 노드인 경우
